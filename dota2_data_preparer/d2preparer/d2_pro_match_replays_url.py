...
def save_match_in_db(matches_json):
    update_statement = pro_match.update()\
        .where(pro_match.c.match_pk == matches_json['match_id'])\
        .values(replay_url=matches_json['replay_url'])
...
def pro_match_predicate(res):
    ...
    return res.status_code != 200 or parse_error or miss_url
...
def get_last_match_pk_where_url_is_null():
    sel = select([pro_match.c.match_pk]) \
        .where(pro_match.c.replay_url == None) \
